...
> [!warning] 注意
>  `min.insync.replicas` 是 Broker/Topic 级别的配置，不是 Spring 配置文件中的参数，需要在 Kafka 服务端或创建 Topic 时指定。
...
> [!tip] Tips 
> 幂等性只保证**单个生产者会话内、单个 Partition** 不重复。跨会话（生产者重启后 PID 变更）或跨 Partition 的去重需要使用**事务消息**。
...
> [!warning] 注意
> 事务消息会显著降低吞吐量，且消费者需要设置 `isolation.level=read_committed` 才能只消费已提交的事务消息，否则会读到未提交的消息。非必要不使用事务，优先考虑幂等性 + 业务侧去重。
...
> [!tip] 可靠性黄金组合
>  生产环境推荐：`acks=all` + `enable.idempotence=true` + 手动提交 Offset + 死信 Topic 兜底。
>  
>  在此基础上根据业务需要决定是否引入事务消息。