...
> [!tip] Tips 
> 开启幂等性（`enable.idempotence=true`）时，`max.in.flight.requests.per.connection` 最大可设为 5，Kafka 会在 Broker 端通过序列号重排保证顺序，兼顾顺序性与吞吐量，推荐使用。
...
> [!warning] 注意
> 方案二中未到期消息会不断重投，消息量大时会造成大量无效消费，且精度取决于消费速度，不推荐在消息量大的场景使用。
...
> [!tip] Tips 
> 墓碑消息并不会立即消失，会在下一次日志压缩时被清除。在此之前消费者仍可消费到它，消费者需要判断 value 为 null 时执行删除逻辑。
